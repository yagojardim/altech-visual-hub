...
export interface SprintsWorkspaceProps {
  /** When set, scope sprints (list + create) to a single project (by slug or uuid). */
  projectSlug?: string;
  /** Hide the outer page header (useful when embedded in a tab). */
  hideHeader?: boolean;
}

export function SprintsWorkspace({ projectSlug, hideHeader = false }: SprintsWorkspaceProps = {}) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (projectSlug) {
        ...
        const s = await listSprintsByProject(project.id);
        setSprints(s);
        setProjects([project]);
      } else {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {projectSlug
                ? "Ciclos de entrega deste projeto."
      ...
      <CreateSprintDialog
        ...
        projects={projects}
        lockProjectId={projectSlug ? projects[0]?.id ?? null : null}
        onCreated={() => void load()}
      />
...
function CreateSprintDialog({
  ...
  useEffect(() => {
    if (open) {
      setForm({
        nome: "",
        project_id: lockProjectId ?? projects[0]?.id ?? "",
        meta: "",
        data_inicio: "",
        data_fim: "",
        status: "Planejada",
